...
const OtherSetting = () => {
  ...
  const checkUpdate = async () => {
    try {
      ...
      //   `${proxyUrl}https://api.github.com/repos/karlorz/auth-gateway/releases/latest`,
      // );

      // Option 2: Use the JSON proxy approach which often works better with GitHub API
      const res = await fetch(
        'https://api.github.com/repos/karlorz/auth-gateway/releases/latest',
        {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            // Adding User-Agent which is often required by GitHub API
            'User-Agent': 'auth-gateway-update-checker',
          },
        },
  ...
  const openGitHubRelease = () => {
    window.open(
      `https://github.com/karlorz/auth-gateway/releases/tag/${updateData.tag_name}`,
      '_blank',
    );
  };
